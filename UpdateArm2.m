function UpdateArm2(x,y,z,Phi,time,plot_end)
global theta1_IK theta2a_IK theta3a_IK theta4a_IK theta2b_IK theta3b_IK theta4b_IK total_stage;
...
[theta1_IK,theta2a_IK,theta3a_IK,theta4a_IK,theta2b_IK,theta3b_IK,theta4b_IK] = solve_IK(x(stage),y(stage),z(stage),Phi);

if stage ~= 1
theta1_delta=(theta1_IK-theta1*rad2deg)/(time*(total_stage-1)); %find the angle difference between current and last stage
theta2_delta=(theta2a_IK-theta2*rad2deg)/(time*(total_stage-1));
theta3_delta=(theta3a_IK-theta3*rad2deg)/(time*(total_stage-1));
theta4_delta=(theta4a_IK-theta4*rad2deg)/(time*(total_stage-1));
end

theta1=theta1_IK*deg2rad; %update theta1 value for this stage
theta2=theta2a_IK*deg2rad;
theta3=theta3a_IK*deg2rad;
theta4=theta4a_IK*deg2rad;


%% DH table
DH1 = [0,(pi/2),L1,theta1_IK*deg2rad];
DH2 = [L2,0,0,theta2a_IK*deg2rad];
DH3 = [L3,0,0,theta3a_IK*deg2rad];
DH4 = [0,-(pi/2),0,theta4a_IK*deg2rad-(pi/2)];
...
if stage ~= 1
    if (EnableVelocityPrint==1||EnableVelocityPrint==2)
        Jacobian_get_velocity(T05(1,4),T05(2,4),T05(3,4),Phi,theta1_delta,theta2_delta,theta3_delta,theta4_delta,(time/(total_stage-1)));
    end
end
...
if plot_end == 1 && stage==total_stage+1
